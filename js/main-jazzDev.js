...
function generateDataTable(tableData, tableConfig, onElement) {
	...
	function allChecks() {
		...
		if (jQuery(onElement + '.jDataTable tbody tr td:first-child input:checkbox').length) {
	...
	if (tableConfig != undefined) {
		...
		if ((tableConfig.selectable != undefined) && (tableConfig.selectable == true)) {
			jQuery(onElement + '.mainDataTable .jDataTable').addClass('selectableRows');
			if (identifierFound) {
				jQuery(onElement + '.mainDataTable .jDataTable').addClass('hasActions');
				jQuery(onElement + '.mainDataTable .jDataTable thead tr').prepend('<th data-orderable="false" class="noExport rowCheckbox"><div class="defaultCheckBox"><input type="checkbox" id="jSelectAllRows_' + uniqueTxt + '" /><label class="hideText" for="jSelectAllRows_' + uniqueTxt + '">Select All</label></div></th>'); // making Header Checkbox
				jQuery(onElement + '.mainDataTable .jDataTable tbody tr').each(function () {
					var val = jQuery(this).find('td:nth-child(' + identifierCol + ')').text();
					jQuery(this).prepend('<td class="noSelect rowCheckbox"><div class="defaultCheckBox"><input type="checkbox" id="' + val.toString().replace(/[^A-Za-z0-9_-]/g, "") + '" /><label for="' + val.toString().replace(/[^A-Za-z0-9_-]/g, "") + '"></label></div></td>');
				}); // making Checkbox on the rows
				// Table for selected Rows
				jQuery(onElement + '.mainDataTable .jDataTable').after('<table id="selectedRows_' + uniqueTxt + '" style="display:none;"></table>');
				allChecks();
				...
				for (var ac = 0; ac < tableConfig.addColumn.length; ac++) {
					...
					jQuery(onElement + '.mainDataTable .jDataTable tbody tr').each(function () {
						if (tableConfig.addColumn[ac].addButtons != undefined && tableConfig.addColumn[ac].addButtons.length > 0) {
							var actionButton = '';
							var identifierTxt = jQuery(this).find('td:nth-child(' + identifierCol + ')').text();
	...
	if (tableConfig != undefined) {
		...
		if (tableConfig.searchable == true) {
			...
			jQuery(document).on('click', '.resetSearch_' + uniqueTxt, function () {
				jQuery('.filterPopupWrapper .fPopupBody .filterCell, .outerFilterContainer .defaultCheckBox').each(function () {
		...
		if (tableConfig.outerFilter != undefined && tableConfig.outerFilter.length > 0) {
			var checkArray = [], thText = '', dataColumn, colName = tableConfig.outerFilter.toLowerCase();
		...
		if ((tableConfig.filters != undefined) && (tableConfig.filters.length)) {
			...
			function rangeOfCol() {
				table.columns('.jfilter_range').every(function (i) {
					var rangeHtml = '<div class="rangeBox rangeMin_' + i + '"><input placeholder="Min" type="text" class="numbersOnly" id="rangeMin_' + i + '"/></div><div class="rangeBox rangeMax_' + i + '"><input placeholder="Max" type="text" class="numbersOnly" id="rangeMax_' + i + '"/></div>';
					...
					jQuery(document).on('keyup change', '#rangeMin_' + i + ', #rangeMax_' + i, function () {
						table.draw();
					});
				});
			} rangeOfCol();
			...
			function dateRangeOfCol() {
				table.columns('.jfilter_date').every(function (i) {
					var dateHtml = '<div class="dateBox dateMin_' + i + '"><input placeholder="From" type="text" class="datepicker dateFrom" readonly id="dateMin_' + i + '" /><div class="dPopup minDatePopup_' + i + '"></div></div><div class="dateBox dateMax_' + i + '"><input placeholder="To" type="text" class="datepicker dateTo" readonly id="dateMax_' + i + '" /><div class="dPopup maxDatePopup_' + i + '"></div></div>';
					...
					jQuery.fn.dataTable.ext.search.push(
						function (settings, data, dataIndex) {
							var dateMin = parseInt(jQuery('#dateMin_' + i).val().replace(/[^0-9]/g, ""));
							var dateMax = parseInt(jQuery('#dateMax_' + i).val().replace(/[^0-9]/g, ""));
			...
			jQuery(document).on('click', '.filterPopupWrapper .btn-clearFilter', function () {
				...
				// Outerfilter trigger
				if (jQuery('.outerFilterContainer .defaultCheckBox').length) {
					var outerFilterString = [];
					if (jQuery('.outerFilterContainer .defaultCheckBox').length) {
						jQuery('.outerFilterContainer .defaultCheckBox').each(function () {
					...
					table.search(outerFilterString, true, false).draw();
				}
				// setTimeout(function(){
...
$(document).ready(function () {
	...
	$.getJSON("http://localhost/datatables/js/config.json", function (jsn) {
		...
		generateDataTable(data4Table2, tblConfigs, elm1); // old sample
	});
