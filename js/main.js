...
function convertIntoNumber(s) {
	let dIndx = s.search(/\./);
	let dAftr = s.charAt(dIndx-1);
	if(isNaN(dAftr)){ // char before . is not Number ? trim from .
		s = s.substring(dIndx+1);
	}
	s = s.replace(/[^0-9.-]/g, ""); // converting string into numbers
	if(s.indexOf('-') !== -1){
		let indx = s.indexOf('-');
		s = s.substring(indx);
	}
...
var fDataTable = {};
function generateDataTable(tableElm, tableData, tableConfig) {
	if (tableElm == undefined || tableElm.length < 1) {
		console.log("No Element defined for table");
		return false; // if element not defined, stop execution 
	}
	if (jQuery(tableElm).length < 1) {
		console.log("Defined Element not found in HTML");
		return false; // if Defined Element not found in HTML, stop execution
	}
	var createTable = false;
	if (tableData == undefined || tableData.length < 1) {
		if(tableData.length < 1) {
			if(jQuery(tableElm).find('.mainDataTable').length < 1){
				jQuery(tableElm).prepend('<div class="col-xs-12 mainDataTable"><table class="fDataTable"><tbody><tr><td>No record Found</td></tr></tbody></table></div>');
			}
			else {
				jQuery(tableElm).find('.mainDataTable').append('<table class="fDataTable"><tbody><tr><td>No record Found</td></tr></tbody></table>');
			}
		}
		if (jQuery(tableElm + ' > table').length < 1) {
			console.log('No Table or Data found');
			return false; // if no Table found on required place
		}
		else {
			jQuery(tableElm + ' > table').addClass('fDataTable').wrap('<div class="col-xs-12 mainDataTable"></div>');
		}
	}
	...
	var uniqueTxt = tableName,
		...
		totalRows = 0,
		rowsLength = 10, // show Rows per page
		pageInfo = true, // page Info where it is like "Showing 1 to 10 of 57"
	...
	if (createTable) {
		...
		if (jQuery(onElement + '.mainDataTable').length < 1) {
			jQuery(onElement).prepend('<div class="col-xs-12 mainDataTable"></div>');
		}
		jQuery(onElement + '.mainDataTable').append(tableHtml);
	}
	totalRows = jQuery(onElement + '.mainDataTable .fDataTable > tbody > tr').length;
	...
	function allChecks() {
		...
		if (jQuery('#selectedRows_' + uniqueTxt + ' tr').length) {
			jQuery(onElement + '.mainDataTable').parent().find('.btn-table-action').removeAttr('disabled');
		}
		else {
			jQuery(onElement + '.mainDataTable').parent().find('.btn-table-action').attr('disabled', 'disabled');
		}
	}
	...
	if (tableConfig != undefined && totalRows > 0) {
		...
		if (!(tableConfig.downloadable == undefined || tableConfig.downloadable == false)) {
			if (fDataTable[tableName].data().length > 0) {
				...
				jQuery(onElement + '.mainDataTable').parent().append(dlPopup);
				...
				if (!jQuery(onElement + '.mainDataTable').next('.tableAfter').length) {
					jQuery(onElement + '.mainDataTable').after('<div class="col-xs-12 '+uniqueTxt+' tableAfter"></div>');
				}
				else { jQuery(onElement + '.mainDataTable').next('.tableAfter').addClass(uniqueTxt); }
				...
				jQuery(document).on('click', '.downloadTblData_' + uniqueTxt, function () {
					jQuery('.dlPopupWrapper.' + uniqueTxt + ' .dlPopupContent').html(downloadPopupHtml);
					jQuery('.dlPopupWrapper.' + uniqueTxt).fadeIn('fast');
				});
				jQuery(document).on('click', '.dlPopupWrapper.' + uniqueTxt + ' .btn-download_' + uniqueTxt, function () {
		...
		if (tableConfig.searchable == true) {
			var searchboxString = '';
			if (!jQuery(onElement + '.mainDataTable').parent().find('.filterArea').length) {
				jQuery(onElement + '.mainDataTable').before('<div class="col-xs-12 '+uniqueTxt+' filterArea"></div>');
			}
			else { jQuery(onElement + '.mainDataTable').parent().find('.filterArea').addClass(uniqueTxt); }
			...
			jQuery(onElement + '.mainDataTable').parent().find('.filterArea').prepend(searchHtml);
		...
		// Add Outer Filter
		if (tableConfig.outerFilter != undefined && tableConfig.outerFilter.columnName != undefined) {
			var checkArray = [], thText = '', dataColumn, colName, otflAppendTo;
			colName = tableConfig.outerFilter.columnName.toLowerCase();
			if (tableConfig.outerFilter.appendTo != undefined && tableConfig.outerFilter.appendTo.length > 0) {
				otflAppendTo = tableConfig.outerFilter.appendTo;
			}
			else {
				jQuery(onElement + '.mainDataTable').parent().prepend('<div class="col-xs-12"><div class="outFilter"></div></div>');
				// prepending DOT for making it jQuery element
				otflAppendTo = '.' + jQuery(onElement + '.mainDataTable').parent().find('.outFilter').attr('class');
			}
			...
			fDataTable[tableName].columns('.outerFilterColumn').every(function () {
				...
				jQuery('.outerAction.' + uniqueTxt + ' .outerFilterText span').text(colName);
				...
				jQuery('.outerAction.' + uniqueTxt + ' .outerFilterContainer').html(checkboxes);
				jQuery('.outerAction.' + uniqueTxt + ' .outerFilterText span').text(tableConfig.outerFilter.columnName.replace(/_/g, " "));
			...
			jQuery('.outerAction.' + uniqueTxt + ' .outerFilterContainer').mCustomScrollbar({
				axis: "y",
				snapOffset: 2,
				scrollInertia: 150,
				mouseWheelPixels: 80
		...
		if ((tableConfig.filters != undefined) && (tableConfig.filters.length)) {
			if (jQuery(onElement + '.mainDataTable').parent().find('.filterArea').length < 1) {
				jQuery(onElement + '.mainDataTable').before('<div class="col-xs-12 '+uniqueTxt+' filterArea"></div>');
			}
			jQuery(onElement + '.mainDataTable').prev('.filterArea').append('<div class="btn btn-secondary tableFilters ' + uniqueTxt + '">Filters</div>');
			if (jQuery(onElement + '.mainDataTable').parent().find('.filterPopupWrapper').length < 1) {
				...
				jQuery(onElement + '.mainDataTable').parent().append(mFilterHTML);
			}
			...
			function checkboxOfCol() {
				fDataTable[tableName].columns('.jfilter_checkbox').every(function (i) {
					...
					if (checkbox.length > 0) {
						...
						filterHTML += '</div>';
						jQuery('.filterPopupWrapper.' + uniqueTxt + ' .fPopupBody .fBodyScroll').append(filterHTML);
			...
			jQuery(document).on('click', '.filterArea .tableFilters.' + uniqueTxt, function () {
				showFilterPopup(jQuery('.filterPopupWrapper.' + uniqueTxt));
				filterPopupHeight(jQuery('.filterPopupWrapper.' + uniqueTxt));
			});

			jQuery(window).resize(function () {
				filterPopupHeight(jQuery('.filterPopupWrapper.' + uniqueTxt));
				filterPopupUpdate(jQuery('.filterPopupWrapper.' + uniqueTxt));
			});

			jQuery(document).on('click', '.filterPopupWrapper.' + uniqueTxt + ' .btn-applyFilter', function () {
				var searchString = [];
			...
			jQuery(document).on('click', '.filterPopupWrapper.' + uniqueTxt + ' .btn-clearFilter', function () {
				jQuery('.outerAction.' + uniqueTxt + ' .outerFilterContainer .defaultCheckBox').each(function () {
					jQuery(this).find('input[type=checkbox]').prop('checked', false);
				});
				...
				// Outerfilter trigger
				if (jQuery('.outerAction.' + uniqueTxt + ' .outerFilterContainer .defaultCheckBox').length) {
...
$(document).ready(function () {
	...
	$.getJSON("http://localhost/datatables/js/config.json", function (jsn) {
		...
		var tblConfigs = jsn;
		// generateDataTable(preTable, emptyData, tblConfigs); // empty data
		// generateDataTable(elm1, emptyData, tblConfigs); // empty data
		generateDataTable(elm1, data4Table2, tblConfigs); //
		// generateDataTable(elm2, data4Table, tblConfigs); //
		// generateDataTable(elm1, data4Table, tblConfigs);
		// generateDataTable(preTable, emptyData, tblConfigs);
		// generateDataTable(preTable, emptyData, tblConfigs);
	});
